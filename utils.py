...
def get_boxes_yolo(tile):
    ...
    tile = (255 - np.array(tile)) / 255.0  # invert image and rescale to 0-1
    tile = (tile * 255).astype(np.uint8)  # rescale to 0-255
    results = model(tile, size=128)  # run inference
